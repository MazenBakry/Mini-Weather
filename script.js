...
async function getForecast(city) {
    const response = await fetch(
        `https://api.weatherapi.com/v1/forecast.json?key=e1a751b51dfa4424a81153818230308&q=${city}&days=3&aqi=no&alerts=no`
    );
    ...
    bigTemperature.textContent = `${myData.current.temp_c} °`;
    ...
    for (let i = 0; i < 3; i++) {
        lowTemps[i].textContent =
            Math.floor(myData.forecast.forecastday[i].mintemp_c) + "°";
        highTemps[i].textContent =
            Math.floor(myData.forecast.forecastday[i].maxtemp_c) + "°";
